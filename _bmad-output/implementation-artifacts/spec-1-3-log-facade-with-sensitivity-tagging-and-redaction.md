...
- `message` is a `StaticString`, not a `String`: it must be a compile-time literal, so no runtime value — sensitive or not — can be interpolated into it. This is deterministic (a compiler error, not a review-dependent convention): every runtime value is forced through `fields`, where tagging is mandatory. Renegotiated post-merge-review 2026-09-16 — see Spec Change Log.
...
- Post-review amendment: `message` changed from `String` to `StaticString` on all four level methods (see Spec Change Log) to make the "no untagged runtime value" guarantee a compile-time fact rather than a documented convention. Verified negatively as well as positively: temporarily added `log.info("leaked: \(secret)")` with a `String`-typed `secret` and confirmed it fails with "cannot convert value of type 'String' to expected argument type 'StaticString'", then removed the scratch code. Re-ran `swift build`, `swift test --filter CoreTests` (26/26 pass, unchanged — no existing call site interpolated a runtime value into `message`), and `swift build -c release --target Core` (exit 0, zero warnings) after the change.

## Spec Change Log

- Triggering finding: post-review, human feedback on the open PR pointed out that the `.publicSafe`/`.sensitive` tagging system only guarantees anything if there is a *deterministic* rule preventing bypass — otherwise the tagging apparatus is complexity without a guaranteed benefit. The as-shipped facade left one bypass: `message: String` was free text, so a runtime value (e.g. a secret) could be interpolated straight into it, invisible to the `fields`-dictionary type-enforcement and uncaught by anything Story 1.8's planned lint rule checks (that rule targets direct `os_log`/`print` bypass of the facade entirely, not misuse of `message` once inside it).
- Amendment: changed `message`'s type on all four level methods from `String` to `StaticString`, added a private `emit` choke point, and converted to `String` internally via `String(describing:)` before building the redacted message.
- Known-bad state avoided: a "safety" system where the tagging discipline was only as good as a human remembering to use `fields` instead of string interpolation — no different in practice from an undocumented convention, despite the type machinery around it.
- KEEP: the `fields: [String: LogSensitivity]` design, `buildMessage`'s redact-before-`.public` approach, and the `emit` choke point all worked as intended and are unchanged by this amendment.
