...
created: '2026-09-21'
status: 'done'
status_detail: "AC1-AC3 landed, reviewed, and run live with full 3-draw medians (diarized bench arm: 10/19 = 53%, equal to the un-diarized control — no total-recall lever from diarizing the bench transcript alone; one meeting-specific ES2002b action-item effect survives). AC4 met via path 2: the maintainer ruled 2026-09-22 that two of ES2004a's three asserted items fall outside the shipped action-item rule (no commitment, no owner) and removed them from expected.json, with the rationale in its notes field. Re-scored, the full pipeline's 3-run median (Story 4.14, PR #107) rises from 14/19 = 74% to 14/17 = 82%, clearing the 80% floor. The languages decision item is kept and remains an open miss for a future prompt arm. Residual: history.jsonl rows for this revision still need the same re-score, coordinated with PR #107 rather than duplicated here."
...
**Residual risks:**
- `Tests/regression/ami/history.jsonl`'s three rows for revision `21a5771be6f33b3186ab7613de15c27a32c352e5` still score against the old 19-item count; they need the same re-score this story computed by hand (11/17, 14/17, 16/17) applied for real with `score.py meeting`, so the committed history matches the fixture it claims to be scored against. Not done here because that file is actively owned by PR #107 (open, not merged) — coordinate or wait for the merge rather than risk diverging/duplicate rows.
- `Tests/regression/ami/README.md`'s "19 expected items" line is now stale (17) but is also in PR #107's active diff; left for that PR or a fast-follow after it merges rather than edited in parallel.
...
- 4 review findings remain deferred (see `deferred` above), all reachable only via malformed or hand-edited fixture data in `RecallBenchFixtureLoader`'s diarized-load path.
