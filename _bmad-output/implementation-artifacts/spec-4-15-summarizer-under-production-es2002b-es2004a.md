...
created: '2026-09-21'
status: 'done'
status_detail: "AC1-AC3 landed, reviewed, and run live with full 3-draw medians (diarized bench arm: 10/19 = 53%, equal to the un-diarized control — no total-recall lever from diarizing the bench transcript alone; one meeting-specific ES2002b action-item effect survives). AC4 met via path 2: the maintainer ruled 2026-09-22 that two of ES2004a's three asserted items fall outside the shipped action-item rule (no commitment, no owner) and removed them from expected.json, with the rationale in its notes field. Re-scored for real against the corrected fixture (PR #107 merged, this story rebased and rewrote history.jsonl's three rows in place with score.py meeting), the full pipeline's 3-run median rises from 14/19 = 74% to 14/17 = 82% (score.py report: 94%, 65%, 82%), clearing the 80% floor. The languages decision item is kept and remains an open miss for a future prompt arm."
...
**Summary:** AC1 (per-meeting diff) and AC2 (ES2004a's three items characterized as never-proposed) are satisfied by direct investigation, written up committed. AC3 (diarized bench arm) is implemented, tested, reviewed, and run live three times, matching the maintainer-amended median-of-three bar: median 10/19 = 53%, identical to the un-diarized control's median, so the arm demonstrates no total-recall lever from diarizing the bench transcript (the first draw, 14/19, read as a match to the full pipeline before the other two corrected it — recorded honestly rather than left standing). A meeting-specific effect survives: ES2002b's action items never drop below 2/4 across three diarized draws and never rise above 1/4 across two un-diarized draws. A follow-up investigation ruled out glossary, attendee context, and `SummarizerConfig` defaults as explanations for the earlier full-pipeline-vs-bench gap (real cache artifacts as evidence) and identified the actual cause: no summarization call in this codebase sets `temperature`, so every call samples at the API default of 1.0. Story 4.14 (PR #107, open, not yet merged) recorded the full pipeline's own 3-run median: 14/19 = 74%, itself short of the 16/19 bar and clearing it on only 1 of 3 runs. **AC4 is now met via path 2**: the maintainer ruled on 2026-09-22 that two of ES2004a's three asserted items (the budget-figures line, the "everyone works on their own task" line) fall outside the shipped action-item rule's commitment/owner requirements and removed them from `expected.json`, with the full rationale written into the fixture's own `notes` field, not fabricated here. Re-scored against the corrected fixture, the full pipeline's 3-run median rises from 14/19 = 74% to 14/17 = 82% — the same three runs, the same rendered notes, a corrected denominator, not a recovered miss. 82% clears the 80% floor. The languages decision item is kept and stays an open, real miss for a future prompt arm; this story does not resolve it. Per the spec's own `Never` constraint, no result was fabricated and the fixture ruling came from the maintainer, not this story's own authority.
...
- `Tests/fixtures/story-4-15-findings.md` (new, committed) — AC1's diff table, AC2's ES2004a characterization, AC3's three-draw diarized-arm result, the temperature-1.0 finding, Story 4.14's three-run median, and the maintainer's fixture ruling with its re-scored numbers.
- `Tests/regression/ami/reference/es2004a/expected.json` — two of three asserted items removed per the maintainer's 2026-09-22 ruling; rationale written into `notes`.
- `Tests/regression/ami/history.jsonl` — the three recorded full-pipeline runs' `ES2004a` rows re-scored against the corrected fixture (`score.py meeting`, no API cost); `run_at`/`revision` unchanged, only `expected_items` and its per-section expected counts.
- `Tests/regression/ami/README.md` — "19 expected items" corrected to 17 (the one instance not tied to a specific historical run's own numbers).
- `Tests/fixtures/recall-bench-output/2026-09-21-{handoff-from-research,full-pipeline-diarized}/` and `.../2026-09-21-diarized/` (new, gitignored) — the real research-session handoff data, a copy of today's full-pipeline-with-fix run's cache (transcript/diarization/attribution for all 5 AMI meetings, copied out of `~/Library/Caches/com.auricle.app/` before a concurrent session's run could overwrite it), and a "shadow root" built from the same data for the diarized arm's three live draws.
...
**Residual risks:**
- `Tests/regression/ami/README.md:41`'s calibration narrative ("three recorded runs... of the same 19 expected items") and `thresholds.json`'s `min_item_recall: 0.68` were both derived from the 19-item count before the fixture ruling. `min_item_recall` is a regression floor (13/19 = 0.68), not this story's 80% exit bar, and recalibrating it against the new 17-item count is a Story 4.14-owned decision this story does not make unilaterally — flagged, not changed.
- The languages-decision miss in ES2004a (kept in the fixture, confirmed real) is unresolved — a candidate for a future single-variable prompt arm, out of this story's scope now that its stop condition is met.
- The diarized bench arm's ES2002b-action-item effect (real, non-overlapping across 5 runs) and its ES2003b volatility (also real, works against the total) are both recorded but not explained mechanistically — worth a follow-up if diarization is revisited as a lever.
- 4 review findings remain deferred (see `deferred` above), all in `Sources/RecallBench/RecallBenchFixtureLoader.swift`'s diarized-load path and reachable only via malformed or hand-edited fixture data — a real risk given this exact story's workflow involves hand-copying fixture directories, but none reachable via the app's own artifact writers today.
