...
**Summary:** AC1 (per-meeting diff) and AC2 (ES2004a's three items characterized as never-proposed) are satisfied by direct investigation, written up committed. AC3 (diarized bench arm) is implemented, tested, reviewed, and now run live: `--diarized` scored 14/19 = 74%, matching the full pipeline exactly, driven by ES2002b's action items recovering from 0-1/4 to 4/4 kept. A follow-up investigation ruled out glossary, attendee context, and `SummarizerConfig` defaults as explanations for the earlier full-pipeline-vs-bench gap (real cache artifacts as evidence) and identified the actual cause: no summarization call in this codebase sets `temperature`, so every call samples at the API default of 1.0 — the prior 10/19-to-14/19 spread on byte-identical input is that variance, not a hidden input (see findings doc). AC4 (the stop condition) is not met: the diarized run reached 74%, short of 16/19, and the remaining path needs a full-pipeline run recorded to `history.jsonl`, which needs Story 4.14's retention metric and rerun (still landing in a separate session as of this writing) — and, given the confirmed variance, is weak evidence from a single run regardless. Per the spec's own `Never` constraint, no result was fabricated and no fixture ruling was made unilaterally.
...
**Residual risks:**
- The diarized bench arm has been run once (14/19), not three times — one data point is consistent with a real diarization effect but not distinguishable from a favorable roll of the confirmed temperature-1.0 variance without repeats.
- The diarized run produced 4 ungrounded quotes, 0 on every un-diarized arm run so far. Recorded in the findings doc as an observation, not investigated further.
