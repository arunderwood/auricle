...
**Finalization outcome:** `git commit` succeeded on the first attempt (all 12 reviewed-diff files: `.swiftlint.yml`, `Package.swift`, both new `Sources/ClaudeSummarizer/` files, `Sources/Summarize/Prompts/summarize/citations.md`, both new `Tests/ClaudeSummarizerTests/` files, `Tests/CoreTests/CanonicalTranscriptContractTests.swift`, `Tests/SummarizeTests/CrossModeFixtureTests.swift`, `Tests/SummarizeTests/PromptBuilderSnapshotTests.swift`, `Tests/SummarizeTests/Snapshots/prompts/citations.txt`, and this spec file) as `7ab8e71`. Pre-commit swiftformat/swiftlint hooks passed cleanly. Working tree verified clean afterward.
