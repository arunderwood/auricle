...
### 2026-09-18 — Narrowed the requested scope and the fields mask

The requested scope became `https://www.googleapis.com/auth/calendar.events.owned.readonly`, narrower than the `calendar.readonly` that the story text names. The only call the source makes is `events.list` on the primary calendar, and NFR-S6 asks for the minimum surface; `calendar.readonly` also reads calendar lists, ACLs and settings. Google's authorization list for `events.list` includes the narrower scope, and the granted-scope check requires it. The `fields` mask asks for `dateTime` only on `start` and `end`, because the source never decodes an all-day event's bare `date`; such an event arrives with no `dateTime` and is still skipped as all-day.
