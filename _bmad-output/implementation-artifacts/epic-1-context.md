...
- Story 1.2: Core Primitives — AtomicWriter, IDs, CanonicalTranscript, Dialects, Config. As built, only AtomicWriter, IDs and Dialects landed here: `CanonicalTranscript` landed in Story 3.1 and `Config` in the Epic 3 follow-through, covering `vault_path`, `meetings_subdir` and the Google keys. Story 9.1 owns the remaining FR58 keys and symlink normalization
...
- Required test coverage at this layer: atomic-write edge cases, meeting-ID resolution, canonical-transcript round-trip (delivered with Story 3.1, not in this epic), JSON round-trip for every Codable contract type, crash-recovery reconciliation, migration correctness.
...
- **Core primitives:** `MeetingID` wraps a 26-char Crockford-base32 ULID; resolver accepts a full ID, a ≥6-char prefix, `current`, or `last`. `CanonicalTranscript` (built in Story 3.1, not in this epic) is the single transcript representation (NFC Unicode, LF, `<Speaker_N>: ` prefixes, UTF-8 byte offsets) and must round-trip byte-identically — load-bearing for later quote-grounding validation.
