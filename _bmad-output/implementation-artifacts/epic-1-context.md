...
- **Dependency discipline:** for a well-specified, general-purpose problem (an encoding, a container/file format, a standard protocol) default to a native Apple framework or an established OSS package rather than hand-rolling — a hand-roll needs a documented reason tied to a real constraint the library doesn't meet ("no library was evaluated" doesn't count). This applies inside `Core` too: a dependency there is an escalation to name and justify, not a default to avoid. `Core`'s only external dependencies are `TOMLKit` and `ULID.swift` (yaslab, MIT, no transitive third-party deps beyond Foundation); any further one is a documented architecture escalation.
- **Core primitives:** `MeetingID` wraps a 26-char Crockford-base32 ULID generated via the `ULID.swift` library (not hand-rolled encode/decode); resolver accepts full ID, ≥6-char prefix, `current`, or `last`. `CanonicalTranscript` is the single transcript representation (NFC Unicode, LF, `<Speaker_N>: ` prefixes, UTF-8 byte offsets) and must round-trip byte-identically — load-bearing for Epic 3's quote-grounding validators.
