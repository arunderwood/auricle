...
- `x-apple.systempreferences:com.apple.settings.PrivacySecurity.extension?Privacy_Microphone` → "Microphone". Correct — matches the candidate already shipped.
- `x-apple.systempreferences:com.apple.preference.security?Privacy_Notifications` (the candidate this PR had shipped) was **not** verified and was replaced, unused, with `x-apple.systempreferences:com.apple.Notifications-Settings.extension` → "Notifications", with its Application Notifications list. Confirmed correct; `remediationDeepLink(for: .notifications)` and its test now use this URL. `architecture.md` Decision 4.4 records all three verified URLs.
...
**Deferral reconciliation:** the "Deep-link verification (2026-09-22)" block above supersedes the earlier "unverified" wording about the deep links, in the frontmatter `deferred` and in this section's verification and deferral lines. The live Notifications-prompt string is still unobserved.
