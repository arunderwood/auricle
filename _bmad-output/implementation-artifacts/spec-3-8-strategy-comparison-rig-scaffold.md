...
**Renamed after finalization:** "smoke test" read as throwaway proof-of-concept code, but this is permanent, hand-run tooling, so the code now says what it does. `SmokeTest*` types and files became `StrategyComparison*`; the hidden verb `__smoke-test-summarize` became `__compare-strategies`; `Tests/scripts/run-smoke-test.sh` became `Tests/scripts/run-strategy-comparison.sh`; `AURICLE_SMOKE_TEST_TRANSCRIPTS` became `AURICLE_COMPARISON_TRANSCRIPTS`; the ignored directories became `Tests/fixtures/strategy-comparison-transcripts/` and `Tests/fixtures/strategy-comparison-output/`. The results document was renamed later, together with this spec file, to `Tests/fixtures/strategy-comparison-results.md`; Story 3.8's acceptance criterion in `epics.md` names the new path. The spec was `spec-3-8-smoke-test-rig-scaffold.md` until then. The sections above, the Story 3.7 spec, and the first entry in `deferred-work.md` still use the old names and are left as written.

**Loader extended after finalization:** `StrategyComparisonFixtureLoader` now reads two mixable layouts: flat `<name>.json` files and `<name>/transcript.json` subdirectories (the layout of `Tests/SummarizeTests/Fixtures/eval`). Other files beside a `transcript.json` are ignored, and a flat file and a subdirectory with the same name are rejected with `duplicateFixtureName`. Fixtures are now sorted by name, not by filename. The empty-directory refusal message names both layouts. The sections above still describe the flat-only loader.
