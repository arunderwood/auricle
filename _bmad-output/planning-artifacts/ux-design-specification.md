...
The headline emotion is **calm trust** — not delight, not excitement. Andrew should walk out of a meeting and not think about the notes. Mental load shifts from "did I capture that?" to "I know it's in there." auricle doing its job correctly should feel like nothing — and that nothing is the entire product.
...
│  [👤 Ben]  [👤 Andrew (you) ✓]  [👤 Priya •unmatched] │  ← coverage strip
├───────────────────────────────────────────────────────┤
│  ┌─ Speaker_1 ──────────────────────────────────┐    │
│  │  ▶  ▁▃▅▇▅▃▁  0:07           [ This is me ]  │    │
│  │  Andrew Underwood (you) ✓  [undo]            │    │  ← heuristic pre-select
│  └──────────────────────────────────────────────┘    │
│  ┌─ Speaker_2 ──────────────────────────────────┐    │
│  │  ▶  ▁▂▄▆▄▂▁  0:09           [ This is me ]  │    │
│  │  [[Ben]]  ✓                                  │    │
...
$ auricle attribute 01HZ7K --speakers "1=Ben,2=Andrew Underwood,3=Priya"
Resolved against vault: ✓ Ben, ✓ Andrew Underwood, ✓ Priya (new wikilink)
...
- **v0.1 walking-skeleton validation** (John's "ship Speaker_N + Fix-attribution link, see if Andrew clicks"). Brainstorm chose cohesive-milestone explicitly; we ship attribution at MVP. But John's underlying critique — *don't polish the toll booth* — is folded into the "framing caveat" at the top of this section.
...
1. Information density over breathing room for the meeting list (Linear-style; Andrew runs 10–25 meetings/week)
...
Andrew installs auricle on a fresh Mac and reaches a state where he can record a meeting.
...
Andrew records a 30-min meeting, names speakers, gets a verified note in Obsidian.
...
sequenceDiagram
    ...
    Note over Andrew: User initiates engagement<br/>(no auto-foreground)
    Andrew->>Main: Click row OR notification OR banner
    Main->>Sheet: Present Attribution sheet
    Note over Sheet: Heuristic pre-fills "Andrew (you)"<br/>on longest-speaking row;<br/>recurring-meeting prefill from FR23
    Andrew->>Sheet: Spacebar (audition Speaker_2)
    Andrew->>Sheet: Type "Be" → Enter (Ben)
    Andrew->>Sheet: Cmd-Enter (Continue)
    ...
    Andrew->>Notif: Click notification
...
```mermaid
flowchart TD
    Stop[Andrew clicks Stop] --> Wait[Pipeline transcribes + reviews diarization ~30s]
    Wait --> Notif[macOS notification: 'Captured — name speakers when ready']
    Notif --> Click[Andrew clicks notification or row]
    ...
    Confirm --> Click2[Andrew clicks confirm]
    ...
    Notif2 --> Later[Andrew clicks at convenience or ignores]
...
sequenceDiagram
    ...
    Andrew->>Sheet: Reads AI reasoning ("question-answer-acknowledgment pattern")
    Andrew->>Sheet: Plays segment 42 audio (▶ Play paragraph)
    Note over Andrew: Confirms — yes, three voices
    Andrew->>Sheet: Click [Apply all] on AI's proposed splits
    Note over Sheet: Segment 42 splits into 3 sub-segments<br/>each with proposed speaker assignment
    Andrew->>Sheet: Adjusts split 2 manually (AI guessed 'unknown')
    Andrew->>Sheet: Cmd-Enter (Continue)
...
Key UX details:
- AI's reasoning is always visible (not hidden behind a tooltip) — Andrew can read *why* and judge plausibility
- Apply / Reject controls are per-segment AND have a global [Apply all] / [Reject suggestions]
- The AI is a hint, not autonomous — every correction passes through Andrew's eyes
...
sequenceDiagram
    ...
    Note over Andrew: Notices banner on next visit
    Andrew->>Main: Click row → expands inline
    ...
    Andrew->>Andrew: Tops up Anthropic credits
    Andrew->>Main: Click [Retry now]
...
Andrew started recording, forgot, 2hr of mostly silence. Click Stop → pipeline pre-flight VAD detects <2 min speech → halts before transcribe. State: `silent` (gray chip). Row click expands inline: "VAD detected only 18s of speech in 2h audio." Actions: [Discard] | [Force process]. No vault note, no notification, no clean-up. MVP fallback (no VAD): pipeline runs, produces thin summary, user manually deletes the note.

#### J4 — CLI fallback (v1.1)

Native attribution sheet has a regression. Andrew opens terminal: `auricle pending` → `auricle attribute 01HZ7K --emit-snippets` → QuickLooks snippets in Finder → `auricle attribute 01HZ7K --speakers "1=Ben,2=Andrew,..."` → resolves names against vault, resumes pipeline. The decoupled-stages architecture is what makes this fallback exist without rebuilding anything.

#### J5 — Retention housekeeping

Andrew sees "Audio deletes in 5 days" on an important meeting. Clicks row → expands inline → ops console shows [Keep audio indefinitely] [Set custom...]. Clicks "Keep audio indefinitely". Inline confirmation. Row annotation updates: "Audio kept (indefinite)". Frontmatter gets `auricle.audio_retention: indefinite`.
...
Tuesday standup with Ben (3rd capture of this calendar event). Both rows pre-filled by FR23 previously-labeled tier (≥3 prior labelings of same calendar attendees). AI review may also catch any drift. Andrew presses Cmd-Enter → done in 1 keystroke. Trust compounds.
...
8. **AI suggestions are visible reasoning, not autonomous corrections** — Claude proposes; Andrew accepts/rejects per item; telemetry tracks for trust calibration.
...
├─────────────────────────────────────────────────────────────────────┤
│  Attendees from calendar:                                           │
│  [👤 Andrew (you) ✓] [👤 Sara] [👤 Marcus] [•Priya unmatched]       │
├─────────────────────────────────────────────────────────────────────┤
│  Speakers (rename applies to all paragraphs unless overridden):     │
│  Speaker_1  ▶ ▁▃▅▇  0:07   [This is me]  Andrew (you) ✓ [undo]    │
...
│   └─ Reassign whole paragraph: [Andrew] [Ben] [Sara…] [+ new]       │
│  ─────────────────────────────────────────────────────────────────  │
│  ...                                                                │
├─────────────────────────────────────────────────────────────────────┤
