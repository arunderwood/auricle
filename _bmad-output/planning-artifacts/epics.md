...
- **FR21 [MVP]:** Andrew can review detected speakers in a native Attribution sheet attached to the main window (modal, not a separate window — single-workflow-window principle) after diarization completes, with the pipeline blocked on his input before summarization runs.
...
- **FR58 [MVP]:** Andrew can configure: vault path, vault subdirectory for meeting notes, default audio retention grace window, summarization engine choice (Claude / local), Anthropic API key, Google OAuth account, log verbosity. Includes `diarization_review.enabled` and `diarization_review.model` per FR74.
...
- **AR-DIST-1:** Self-managed code-signing CA + per-Mac `spctl` trust policy. Personal "Andrew Code Signing CA" + "Auricle Code Signing" leaf cert; CA `.cer` checked in at `assets/AndrewCodeSigningCA.cer`; private keys remain on originating Mac only.
...
- **AR-DATA-8:** Filename convention: `<YYYY-MM-DD>-<slug>.md` where date is local-timezone capture date and slug is derived via priority chain (calendar event title → `with-<attendees>` → `meeting-at-<HHMM>` → `meeting-<id-prefix>`). Slug normalization: NFKD → strip non-ASCII → lowercase → kebab → 60-char cap at hyphen boundary. Same-Mac collisions append `-2`/`-3` ordinal counter. Cross-Mac collisions accepted as low-probability, manually recoverable.
...
> **Renamed from "On-Device Audio → Vault Note (CLI-Dogfoodable Milestone)" per John's review** — this is a *Pipeline Validation* checkpoint for Andrew-the-builder, not a user-shippable milestone for Andrew-the-meeting-haver. Pre-existing audio + CLI is not a meeting workflow. The cohesive-MVP commitment lives at Epic 9.
...
- Concrete invocation: `auricle run <id> --publish-anyway` against ≥5 of Andrew's existing meeting recordings (the same fixture set used in Epic 3's smoke-test).
...
- **Story 1 blocker resolved (Amelia):** all wedge-validation telemetry counter columns wired into `telemetry` table in Story 1.4 — no future epic needs a migration to add a column
...
**And** the slug is derived via priority chain (try each, fall through if it produces empty/unusable slug): (1) calendar event title; (2) `with-<attendee-1>-and-<attendee-2>` for ≤3 attendees with named attribution, omitting self-attribution (1:1 with Ben → `with-ben`); (3) `meeting-at-<HHMM>` generic fallback with 24h local time, 4 digits no separator; (4) `meeting-<first-8-chars-of-id>` defensive terminal fallback
...
**Then** ≥5 of Andrew's existing meeting recordings are transcribed via WhisperKit (placed under `tests/fixtures/smoke-test-transcripts/` or a path-referenced env var per NFR-M5)
...
**Given** the future maintainer (or future-Andrew)
...
**Given** Andrew is calibrating his trust in the validator (J1.5)
...
**Given** future-Andrew adds a new captured-meeting fixture
...
> Renamed per John's review — this is a *Pipeline Validation* checkpoint for Andrew-the-builder, not a user-shippable milestone for Andrew-the-meeting-haver. **This is a gate, not a ship.** The cohesive-MVP commitment lives at Epic 9.
...
**When** `auricle attribute <id> --speakers "1=Ben,2=Andrew Underwood,3=Priya"` is invoked (CLI batch mode)
...
**Given** the smoke-test fixture set (≥5 of Andrew's existing meeting recordings, overlapping with Story 3.8's smoke-test set is fine)
...
**Then** the fixture set is the same one Andrew used in Story 3.8 (smoke-test default selection) and Story 3.9 (eval harness regression) — single source of truth for "Andrew's real meetings the validators have been measured against"
...
**When** Andrew reviews the test output
...
**And** the menu item / hotkey is wrapped in `#if DEBUG` or guarded by a debug build configuration so it does NOT appear in release builds (Andrew can still produce a debug-built `.app` for end-to-end Epic 5 testing on real Macs)
...
**Given** Andrew launches auricle for the first time on a fresh Mac
...
**Then** the form pre-fills with a best-guess: the macOS system account full name (e.g., from `NSFullUserName()`) wrapped in `[[...]]` (e.g., `[[Andrew Underwood]]`)
...
**When** Andrew is figuring out the affordance
...
**Given** the heuristic pre-selects the wrong row (rare on 1:1s; more common on multi-party where Andrew isn't the dominant speaker)
**When** Andrew clicks `[undo]` on the heuristic-pre-filled row
...
**And** the glyph is **non-blocking** — it does NOT gate `[Continue]` per UX-DR36 (Andrew can play the snippet and judge for himself; manual merge is v1.1)
...
So that every Apply action is reversible — the AI is a hint, not autonomous; every correction passes through Andrew's eyes per UX spec Step 10.
...
**Then** the artifacts exist: "Andrew Code Signing CA" certificate (root, self-signed, marked CA-capable, generated via Keychain Access > Certificate Assistant or `openssl`); "Auricle Code Signing" leaf certificate (signed by the CA, used as Xcode signing identity)
**And** the CA `.cer` (public certificate, no private key) is checked into the repo at `assets/AndrewCodeSigningCA.cer`
...
**Then** the script is idempotent: imports CA cert as trusted root in System.keychain via `sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain`; computes SHA-256 fingerprint of the CA cert via `openssl x509 -fingerprint -sha256`; registers Gatekeeper assessment policy via `sudo spctl --add --type execute --label 'AndrewCA' --requirement 'anchor H"$CA_HASH"'`
...
**Given** the script
**When** I inspect it
**Then** the script runs: `xcodebuild archive` for the GUI scheme; `codesign` with the leaf cert chained to "Andrew Code Signing CA" (per AR-DIST-3); package as `.dmg` (or `.app` zip) per AR-DIST-3
...
**Given** `--speakers "1=Ben,2=Andrew Underwood,3=Priya"`
**When** invoked
