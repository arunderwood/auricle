...
**Given** the script
**When** I inspect it
**Then** the script runs: `mise install`; `tuist generate --no-open` (the project is generated, not committed, per AR-INIT-1); `xcodebuild archive` for the GUI scheme; `codesign` with the leaf cert chained to "Andrew Code Signing CA" (per AR-DIST-3); package as `.dmg` (or `.app` zip) per AR-DIST-3
