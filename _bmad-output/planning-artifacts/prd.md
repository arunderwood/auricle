...
- **FR21 [MVP]:** Andrew can review detected speakers in a native Attribution sheet attached to the main window (modal, not a separate window — per the single-workflow-window principle) after diarization completes, with the pipeline blocked on his input before summarization runs.
