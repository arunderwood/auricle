...
- **NFR-C1 [MVP]:** Per-meeting variable cost (Claude API for summarization) target ≤ **\$0.50** for a 30-minute meeting with attendee context and vault glossary, at the default model selection (`claude-opus-4-7` with moderate extended-thinking effort budget per NFR-I6) and with prompt caching enabled for the system prompt and stable glossary terms. This ceiling reflects the cost characteristics of Opus-tier reasoning (intentionally chosen for higher-quality grounding per NFR-I6) and is empirically validated in the summarization spike (see Open Resolutions). Cost-control knobs available to the user via configuration: (a) reduce extended-thinking effort budget toward zero (lowest effort approaches a no-thinking baseline of approximately ~\$0.15/meeting); (b) switch summarization model to a smaller variant (Sonnet, Haiku) at the cost-vs-quality trade-off; (c) v1.1+ switch to local-LLM path (FR33) for \$0/meeting.
