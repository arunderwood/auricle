...
### Persona: Andrew, the single user
...
**Opening scene.** Tuesday morning. Andrew has a 30-minute Zoom with a collaborator. Five minutes before the meeting starts, he opens auricle (Dock icon, main window appears), confirms the calendar shows the upcoming event in the sidebar, and that's the entire pre-meeting interaction. He could also have done nothing — auricle's window can stay closed, and recording can be started from the menubar in v1.1, or via `auricle record` from a terminal at any time.

**Rising action.** The meeting starts. Andrew clicks the prominent **Record** button in the auricle main window. A small recording indicator appears (a pulsing red dot in the title bar of the auricle window; in v1.1, also in the menubar). Andrew puts auricle out of his mind. He participates in the meeting normally — speaks, listens, takes no manual notes. Other participants have no indication that anything is being recorded; auricle is invisible to Zoom and to them.

**Climax.** The meeting ends. Andrew clicks **Stop**. The auricle window now shows the meeting in a "Processing" state with a small progress indicator (transcribing → diarizing → ready for attribution). Roughly 30–60 seconds later, the window switches to the **Attribution** view: a list of detected speakers (`Speaker_1`, `Speaker_2`, `Speaker_3`), each with a 5–10 second representative audio snippet (play button, QuickLook-style), and an autocomplete input for each. Andrew clicks the play button on `Speaker_1`, hears his collaborator's voice, types "Be" — the autocomplete shows `Ben` (calendar-attendee-marked, top of list, visually distinct from generic vault wikilinks below), Andrew presses Tab. He repeats for `Speaker_2`, which is himself; he selects his own wikilink, also calendar-marked. There were only two speakers. He clicks **Continue**. The summarization stage runs. ~30–60 more seconds. A macOS notification appears in the top-right: **"auricle: meeting ready — Tuesday sync with Ben."**

**Resolution.** Andrew clicks the notification. Obsidian launches (or focuses, if already running) and opens the new note. The note is at `~/checkouts/SecondBrain/Meetings/2026-04-28-tuesday-sync-with-ben.md` (or whatever the locked-in path convention is). The frontmatter contains the meeting time, duration, attendees as wikilinks, source path to the audio file, and a `auricle:` block with pipeline metadata. The body shows: a one-paragraph summary, an `## Action Items` section with three bullets each carrying a `> source quote`, a `## Decisions` section with one bullet and quote, and a collapsed `## Transcript` section at the bottom. Andrew skims, edits one action item's wording, and closes the file. In the auricle main window, the meeting now shows as ✓ Verified, with "Audio deletes in 7 days" as a small annotation. Andrew closes the auricle window. The app stays alive in the Dock.
...
- Attribution UI: per-speaker snippet playback, calendar-marked autocomplete, "I am Andrew" self-attribution flow
...
**Opening scene.** Friday afternoon, end of the week. Andrew just finished a 50-minute mostly-listening meeting with eight participants — a project kickoff with a vendor team he's never worked with. He has 8 minutes until his next call. He clicks Stop in auricle.

**Rising action.** auricle moves to the Attribution view. There are seven detected speakers (one person spoke too briefly to be diarized as their own cluster). Andrew recognizes his own voice and one collaborator. The other five voices are people he just met today — he'd be guessing at names, and the autocomplete is showing six calendar attendees but he can't reliably map snippet-to-name without significant playback effort. He doesn't have the time.

**Climax.** Andrew clicks the **"Publish anyway"** button at the bottom of the Attribution view. A small confirmation appears: *"This meeting will be published with `Speaker_N` labels and tagged `#auricle/needs-attribution` in frontmatter. You can fix it in Obsidian later, or re-run attribution from this window."* Andrew clicks Confirm. Summarization runs. ~60 seconds later, the notification appears.

**Resolution.** Andrew clicks the notification. Obsidian opens the note. Speakers in the summary text are rendered as `Speaker_1`, `Speaker_2`, etc. (no wikilinks). The frontmatter contains `tags: [auricle/needs-attribution]`. The action items still have quote-grounding; the decisions are still there. The note is usable for end-of-day recall even though the speakers aren't named. Andrew doesn't fix attribution today — but a week later, when he's revisiting the project, he opens the note, recognizes a quote, edits `Speaker_3` to `[[Priya]]` everywhere in the file, and removes the `#auricle/needs-attribution` tag manually. The note is now permanently fixed.
...
**Opening scene.** Andrew started recording before joining a Zoom call, then realized he was on the wrong meeting URL and the actual meeting got rescheduled. He forgot the recording was running. Two hours later, he notices the auricle recording indicator and clicks Stop.
...
**Opening scene.** Andrew is on macOS Sonoma. A new auricle release introduces a regression in the Attribution view — the snippet playback button doesn't respond to clicks. He has a meeting that finished an hour ago waiting for attribution. He files a bug, but he wants to publish the note now.

**Rising action.** Andrew opens a terminal and runs `auricle pending`. The output lists the stuck meeting with its ID. He runs `auricle attribute <id> --emit-snippets`, which writes seven WAV snippets to `~/Library/Caches/auricle/<id>/snippets/`. He QuickLooks each one in Finder, recognizes voices, and writes down the mapping mentally.

**Climax.** Andrew runs `auricle attribute <id> --speakers "1=Ben,2=Andrew,3=Priya,4=Marcus,5=Diana,6=Kenji,7=Sara"`. The CLI resolves each name against the vault (matching wikilink targets, falling back to creating new wikilinks for unknown names), validates the mapping, and resumes the pipeline. Summarization runs. The notification fires ~60 seconds later.

**Resolution.** Andrew clicks the notification, opens the note in Obsidian. Everything is correctly attributed — the broken UI never blocked him. He continues using the released CLI path until a fix ships.
...
**Opening scene.** Three months in, Andrew has used auricle through a contentious project meeting. The summary captured the decisions correctly, but he wants to keep the audio long-term as a reference — there's institutional value in being able to re-transcribe if the summary ever gets disputed. The default retention timer would delete it 7 days after he clicked the verification notification.

**Rising action.** Andrew sees the meeting in the auricle main window with "Audio deletes in 5 days" annotation. He right-clicks (or in v1.1, runs `auricle keep <meeting-id>`). A menu appears with **Keep audio indefinitely** and **Set custom retention…** options.

**Climax.** Andrew clicks **Keep audio indefinitely**. The retention timer is cleared. The annotation now reads "Audio kept (indefinite)". The meeting frontmatter in Obsidian gets a new field: `auricle.audio_retention: indefinite`.

**Resolution.** The audio remains on disk. Six months later, when the dispute actually surfaces, Andrew runs `auricle process <id> --reattribute` (a hypothetical CLI verb worth designing), and the audio is still there. Worst case, it's there for him to play back manually.
...
- **Adoption / replacement claim:** Validated by self-attestation at 30, 60, 90 days. The signal is whether Andrew defaults to auricle's notes when looking up "what did we decide about X" rather than scrolling Slack — a behavioral signal, not a survey.
...
- **Innovation-validation risk (does the architectural bet pay off?).** Mitigation: the user *is* the customer. There's no need to validate market fit in the abstract — adoption-by-self at the 30/60/90 day marks is the validation, and if the tool isn't earning its place in Andrew's workflow, that's a signal to either pivot or shelve. No external commitments are at stake.
...
- **MVP:** Manual download from GitHub Releases. Each release ships a notarized, code-signed `.app` (or `.dmg`). The user replaces the app bundle in `/Applications`. No in-app update prompting in MVP. This is acceptable for a single-user tool the user maintains themselves.
- **v1.1:** Sparkle (the canonical macOS auto-update framework). EdDSA-signed appcasts published from GitHub Releases. In-app "Check for Updates…" menu item. Optional automatic-check toggle (default off — the user retains control). Sparkle's update flow handles the entire download / verify / replace / relaunch sequence and integrates cleanly with code-signing and notarization.
- **Code signing:** Developer ID Application certificate (Apple Developer account). Notarization via `notarytool`. Hardened runtime enabled. This is **required** even for personal-use distribution because macOS Gatekeeper will refuse to launch unsigned `.app` bundles without explicit user override; for a tool intended to be reliable, that friction is unacceptable.
...
- **Build / packaging:** Swift Package Manager for source dependencies (WhisperKit, swift-argument-parser for the CLI, Sparkle for v1.1). Xcode project for the app target; CLI target as a secondary executable in the same project. Universal binary not needed (Apple Silicon only). Notarization via `xcrun notarytool` in the release script.
...
- **Team:** One engineer (Andrew). Evenings and weekends. The brainstorm explicitly tested both "weekend MVP" and "evening MVP" pressure scenarios in Phase 4 — both produced the same opinionated cut list.
...
- **Hardware:** M1 minimum (Andrew has M5 Max, well above the floor). No external infrastructure required — no servers, no databases, no CDN, no telemetry endpoints.
- **External dependencies (paid):** Apple Developer account (\$99/year, required for Developer ID code signing and notarization). Anthropic API account with usage-based billing (~\$0.10 per 30-min meeting at MVP cost ceiling). Google Cloud project for Calendar OAuth client (free at this volume). Total recurring cost: ~\$10/month at typical meeting load.
...
- **FR1 [MVP]:** Andrew can start audio capture for a meeting via a prominent control in the auricle main window.
- **FR2 [MVP]:** Andrew can stop audio capture via the same control, ending the recording and triggering the post-capture pipeline.
- **FR3 [MVP]:** Andrew can see a visible recording-state indicator while capture is active (in the main window title bar, at minimum).
...
- **FR7 [MVP]:** Andrew can manually discard a captured-but-unprocessed meeting from the main window, removing the cached audio.
- **FR8 [v1.1]:** Andrew can start and stop capture from a menubar item without opening the main window.
...
- **FR10 [v1.1]:** Andrew can override a VAD halt and force-process a meeting via UI affordance and CLI flag (`auricle process <id> --force`).
...
- **FR13 [MVP]:** Andrew can see a per-meeting state in the main window indicating which stage is in progress, awaiting user action, or complete (`Recording`, `Processing`, `Awaiting Attribution`, `Awaiting Verification`, `Verified`).
- **FR14 [MVP]:** Andrew can re-run a failed stage idempotently without corrupting artifacts from earlier stages.
- **FR15 [v1.1]:** Andrew can list all in-flight, silent, awaiting-attribution, awaiting-verification, and stale-pending meetings via `auricle pending`.
...
- **FR21 [MVP]:** Andrew can review detected speakers in a native attribution UI panel after diarization completes, with the pipeline blocked on his input before summarization runs.
- **FR22 [MVP]:** Andrew can play a short representative audio snippet (5–10 seconds) for each detected speaker via an in-UI playback control.
- **FR23 [MVP]:** Andrew can assign a name to each detected speaker via an autocomplete input that prioritizes (1) calendar attendees of the current meeting (visually marked), (2) existing vault wikilink targets, (3) previously-labeled speakers, with frequency/recency tie-breakers.
- **FR24 [MVP]:** Andrew can mark himself as a specific speaker without typing — a "this is me" affordance.
- **FR25 [MVP]:** Andrew can publish a meeting without completing attribution via a "Publish anyway" action; the resulting note uses `Speaker_N` placeholder labels and is tagged `#auricle/needs-attribution` in frontmatter.
- **FR26 [MVP]:** Andrew can manually fix attribution in Obsidian after the fact (auricle never re-edits the note, so manual fixes are permanent and uncontested).
- **FR27 [v1.1]:** Andrew can complete attribution via CLI when the native UI is unavailable: `auricle attribute <id> --emit-snippets` writes WAV snippets to the cache directory; `auricle attribute <id> --speakers "1=Ben,2=Sara,..."` accepts a manual mapping and resumes the pipeline.
...
- **FR29 [MVP]:** Each action item and each decision in the structured output includes a `source_transcript_quote` field containing a verbatim excerpt from the source transcript.
- **FR30 [MVP]:** auricle can validate every `source_transcript_quote` by literal substring match against the source transcript, dropping any item whose quote does not validate, before the note is written to the vault.
- **FR31 [MVP]:** auricle can summarize via the Anthropic Claude API as the default summarization engine.
- **FR32 [MVP]:** auricle can use a single constrained-JSON Claude call per meeting (chain-of-summarize is explicitly deferred).
...
- **FR43 [MVP]:** Andrew can click the notification to open the resulting note in Obsidian via URL scheme.
...
- **FR48 [MVP]:** Andrew can see per-meeting audio retention status (e.g., "Audio deletes in 5 days", "Audio kept (indefinite)", "Audio deleted") in the main window.
- **FR49 [v1.1]:** Andrew can override audio retention for a specific meeting (indefinite or custom window) via the main window UI and via `auricle keep <meeting-id>` CLI.
- **FR50 [v1.1]:** Andrew can configure per-meeting retention windows that override the default.
...
- **FR58 [MVP]:** Andrew can configure: vault path, vault subdirectory for meeting notes, default audio retention grace window, summarization engine choice (Claude / local), Anthropic API key, Google OAuth account, log verbosity.
...
- **FR64 [MVP]:** Andrew can quit the auricle app entirely via standard Cmd-Q, which gracefully stops any active capture and persists in-flight state.
- **FR65 [v1.1]:** auricle can self-update via Sparkle, fetching signed updates from the project's release feed with user confirmation.
...
- **NFR-S2 [MVP]:** auricle binaries are signed with a Developer ID Application certificate and notarized by Apple before distribution. Hardened runtime is enabled.
...
- **NFR-I6 [MVP]:** auricle integrates with the Anthropic Messages API. The model identifier is configurable; default at MVP is the latest Claude Sonnet (currently `claude-sonnet-4-6`). Updating the default to a newer model is a release-note bump, not a frontmatter-schema change.
...
- **NFR-C1 [MVP]:** Per-meeting variable cost (Claude API for summarization) ≤ **\$0.10** for a 30-minute meeting with attendee context and vault glossary, at the default model selection.
...
- **NFR-C4 [MVP]:** Total fixed cost (Apple Developer account: \$99/year) is the only non-variable expense. No subscription dependencies, no SaaS components.
...
- **Default Claude model.** Currently `claude-sonnet-4-6` per the user's environment context. New Sonnet/Opus releases should be evaluated for the cost/quality trade-off before becoming the new default.
