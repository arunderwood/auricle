...
```
OLD:  - Personal "Auricle Root CA" certificate — root, self-signed, marked CA-capable (Basic
...
NEW:  - Personal "Auricle Root CA" certificate — root, self-signed, marked CA-capable (Basic
...
```
OLD:  **Then** the artifacts exist: "Auricle Root CA" certificate (root, self-signed, marked
...
NEW:  **Then** `scripts/create-signing-ca.sh` (idempotent, per AR-PAT-11) generates both artifacts with no
        GUI step: "Auricle Root CA" (root, self-signed, `basicConstraints=critical,CA:TRUE`,
...
OLD:  **Then** the script runs: `xcodebuild archive` for the GUI scheme; `codesign` with the leaf cert
        chained to "Auricle Root CA" (per AR-DIST-3); package as `.dmg` (…)
