...
`max_false_keeps` is 6 against a re-scored baseline of 4: one item is 6.7 percentage points at this set size, so a tight bound would fire on noise. `min_item_recall` is still 0.35, not yet raised; see below for why.
...
`report` gates item recall, the per-section split and false keeps on the newest `run_at` in `history.jsonl` only — a real regression in the latest run must not be able to hide behind older rows that will never be re-scored. Older rows still print, labelled, for context. Naming a single meeting on the command line checks it against a limit calibrated for the five-meeting set, not the one meeting — ES2004a alone is 0%, which would fail that limit even though nothing in that run regressed. Read a one-meeting run for its WER, realtime factor and cost, and judge recall from a full run.

`min_item_recall` stays 0.35, unraised. The 2026-09-22 run (revision `e88e2e542ac2c73b9f8265d512c81561e0585bc4`) recorded the first full-pipeline row under the unset first-token gate: 74% (14/19) — 100% on ES2002a, ES2003a and ES2003b, 75% on ES2002b, 0% on ES2004a. But the summarization call sets no `temperature` (`claude-opus-5` accepts none), so repeated runs over byte-identical input are not reproducible: three runs of the same fixed transcript set have scored 10, 10 and 14 of 19. One draw cannot calibrate a floor. The maintainer's decision is to gate on the median of three recorded full-pipeline runs under the current revision, with the existing bar of 16/19, and to raise `min_item_recall` from that median once three exist — this run is the first of those three.
