...
6. Build the two lists independently. Work through the transcript once for action items and again for decisions. How much you found for one list says nothing about the other: a long decision list is not a reason to report fewer action items, and an empty one is not a reason to report more.
7. If a glossary is given below, use it to disambiguate names and terms; if a term in the transcript matches a glossary entry, render it accordingly. If no glossary is given, do not mention it.
8. Output a one-paragraph summary, then arrays of action items and decisions.
