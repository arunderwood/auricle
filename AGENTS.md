...
- `make check` (or `scripts/check.sh`) runs the full gate chain exactly as `ci.yml` does — swiftformat, swiftlint, the custom-lint-rule fixture self-check, the AMI scorer self-check, actionlint, zizmor, `swift build`/`swift test` with `--explicit-target-dependency-import-check error`, a release build, both Xcode schemes, and an assertion that the built `AuricleApp.app` embeds an executable `auricle-cli`. Run it before pushing — it's the same script CI invokes, not a parallel copy that can drift.
