...
public struct MeetingID: Hashable, Sendable {
    ...
    public init?(ulid: String) {
        guard ULIDFormat.isValid(ulid) else { return nil }
        rawValue = ulid.uppercased()
    }

    /// Generates a fresh, timestamp-ordered `MeetingID`.
    public static func generate() -> MeetingID {
        // ULIDFormat.generate() always produces a well-formed 26-char ULID,
        // so the validating initializer can't fail here.
        MeetingID(ulid: ULIDFormat.generate())!
    }
}
